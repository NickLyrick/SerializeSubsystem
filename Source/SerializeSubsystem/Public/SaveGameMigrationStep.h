...
/**
 * Registers a CoreRedirect for the duration of the load so that the old
 * property name in the archive resolves to the new property name in current
 * code transparently, without any custom OnSerialize logic.
 *
...
/**
 * After actor properties are deserialized, overrides a named property with
 * ExportedDefaultValue on every actor that IsA(OwnerClass).
 *
 * Use when a new UPROPERTY(SaveGame) is introduced in TargetVersion and the
 * CDO default is not the right starting value for data migrated from older
 * saves. Applied unconditionally to all matching actors; do not combine with
 * FMigration_RenameField for the same property in the same version.
 *
 * ExportedDefaultValue uses UE's text export format — the same as
 * Details-panel Copy Value as Text (e.g. "100.0" for float,
 * "(X=0,Y=0,Z=1)" for FVector).
 */
USTRUCT(BlueprintType)
struct SERIALIZESUBSYSTEM_API FMigration_SetDefaultValue
    : public FSaveGameMigrationStep {
  GENERATED_BODY()

  /** Class whose instances receive the override. Matches IsA so subclasses
   *  are included. */
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Migration")
