...
void BreakpointWithError(FFrame &Stack, const FText &Text) {
  const FBlueprintExceptionInfo ExceptionInfo(
      EBlueprintExceptionType::Breakpoint, Text);

  const int32 BreakpointOpCodeOffset =
      Stack.Code - Stack.Node->Script.GetData() - 1;
  const UEdGraphNode *Node =
      FKismetDebugUtilities::FindSourceNodeForCodeLocation(
          Stack.Object, Stack.Node, BreakpointOpCodeOffset, true);

  struct Local {
    static void
    OnMessageLogLinkActivated(const class TSharedRef<IMessageToken> &Token) {
  ...
  FBlueprintCoreDelegates::ThrowScriptException(Stack.Object, Stack,
                                                ExceptionInfo);
}
#endif
...
bool USaveGameFunctionLibrary::SerializeActorTransform(
  ...
  if (Archive.IsValid() && IsValid(Actor)) {
    ...
    return (bIsLoading || bIsMovable) &&
           Archive.SerializeField(
               TEXT("ActorTransform"), [&](FStructuredArchive::FSlot Slot) {
                 ...
                 if (bIsLoading && bIsMovable) {
                   ...
                   Actor->SetActorTransform(ActorTransform, false, nullptr,
...
#if WITH_EDITOR
  if (ValueProperty &&
      (!ValueProperty->HasAnyPropertyFlags(CPF_Edit) ||
       ValueProperty->HasAnyPropertyFlags(CPF_BlueprintReadOnly))) {
    BreakpointWithError(
        Stack, FText::Format(NSLOCTEXT("SaveGame",
    ...
    Archive.SerializeField(
        ValueProperty->GetFName(), [&](FStructuredArchive::FSlot Slot) {
...
int32 USaveGameFunctionLibrary::UseCustomVersion(FSaveGameArchive &Archive,
  ...
  if (Archive.IsValid() && IsValid(VersionEnum)) {
    ...
    const FGuid VersionId =
        GetDefault<USaveGameSettings>()->GetVersionId(VersionEnum);

    if (VersionId.IsValid()) {
      if (UnderlyingArchive.IsLoading()) {
        // If the archive has one, return its saved version
        const FCustomVersion *CustomVersion =
            UnderlyingArchive.GetCustomVersions().GetVersion(VersionId);
      ...
      UnderlyingArchive.SetCustomVersion(VersionId, Version,
                                         VersionEnum->GetFName());
      return Version;
    }
  }
