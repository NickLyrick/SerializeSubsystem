...
FSaveGameArchive::FSaveGameArchive(FStructuredArchive::FRecord &InRecord,
                                   UObject *InObject)
    : Record(&InRecord), Object(InObject), StartPosition(0), EndPosition(0) {
  ...
  if (Archive.IsLoading()) {
    ...
    // Store our true end position, so that when we destruct, we can fall off
    // the end gracefully
    EndPosition = Archive.Tell();

    // If we have any properties that were redirected in CoreRedirects, fix them
    // here
    for (auto It = Fields.CreateIterator(); It; ++It) {
      TPair<FName, uint64> &Field = *It;

      for (UStruct *CheckStruct = Object->GetClass(); CheckStruct;
           CheckStruct = CheckStruct->GetSuperStruct()) {
        FName NewProperty =
            FProperty::FindRedirectedPropertyName(CheckStruct, Field.Key);
...
FSaveGameArchive::~FSaveGameArchive() {
  ...
  ON_SCOPE_EXIT { Archive.MarkScriptSerializationEnd(Object.Get()); };
  ...
  // If we had any ordering changes or removals of fields, be sure to continue
  // on from the very end
