 ...
 * SoftObjectPath. Also has a utility for redirecting those references (used for
 * redirecting spawned actors).
 */
...
struct TSaveGameProxyArchive final : public FNameAsStringProxyArchive {
  TSaveGameProxyArchive(FArchive &InInnerArchive)
    ...
    // Setting this hints a Serialize method to only serialize SaveGame
    // properties
    ArIsSaveGame = true;
  }

  /** Allows the archive to redirect any object (used for redirecting spawned
   * actors). */
...
private:
  ...
  template <typename ObjectType> FArchive &SerializeObject(ObjectType &Value) {
    FSoftObjectPath Path;
